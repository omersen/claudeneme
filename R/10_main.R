...
main_complete_pipeline <- function(items_csv_path,
  ...
  embedding_result <- run_embeddings(
  ...
  semantic_dass42 <- compute_semantic_metrics(
    1:42, embedding_result$S_cosine, embedding_result$E_norm, community_assignment
  )
  semantic_dass21 <- compute_semantic_metrics(
    data_prep$dass21_indices, embedding_result$S_cosine, embedding_result$E_norm, community_assignment
  )
  semantic_genie <- compute_semantic_metrics(
    genie_ids, embedding_result$S_cosine, embedding_result$E_norm, community_assignment
  )
  ...
  write.csv(
    coverage_table,
    file.path(output_dir, "tables", "04_item_coverage.csv"),
    row.names = FALSE
  )
  ...
  adaptive_null_genie <- run_adaptive_null_benchmark(
    S_cosine = embedding_result$S_cosine,
    E_norm = embedding_result$E_norm,
    subscale_map = data_prep$subscale_map,
  ...
  percentile_genie <- compute_percentile(semantic_genie, adaptive_null_genie$null_dist)

  write.csv(
    semantic_profile,
    file.path(output_dir, "tables", "05_semantic_profile.csv"),
    row.names = FALSE
  )
  write.csv(
    percentile_dass21,
    file.path(output_dir, "tables", "06_adaptive_percentiles_dass21.csv"),
    row.names = FALSE
  )
  write.csv(
    percentile_genie,
    file.path(output_dir, "tables", "07_adaptive_percentiles_genie.csv"),
    row.names = FALSE
  )
  write.csv(
    genie_result$diagnostic$final_items_df,
    file.path(output_dir, "tables", "03_genie_final_items.csv"),
    row.names = FALSE
  )
  writeLines(
    genie_result$report,
    file.path(output_dir, "logs", "02_genie_diagnostic_report.txt")
  )

  umap_plot <- plot_umap_embeddings(
    E_norm = embedding_result$E_norm,
    items_df = data_prep$items_df,
    selected_ids = genie_ids
  )
  if (!is.null(umap_plot)) {
    ggplot2::ggsave(
      filename = file.path(output_dir, "figures", "02_umap_embeddings.pdf"),
      plot = umap_plot,
      width = 10,
      height = 8
    )
  ...
  if (is.null(data_prep$data_clean)) {
    ...
    return(list(
      ...
      adaptive_null_genie = adaptive_null_genie,
  ...
  write.csv(
    ...
    file.path(output_dir, "tables", "08_psychometric_consistency.csv"),
    row.names = FALSE
  )
  write.csv(
    robustness$summary,
    file.path(output_dir, "tables", "09_repeated_split_summary.csv"),
    row.names = FALSE
  )
  write.csv(
    integration$integration_table,
    file.path(output_dir, "tables", "10_integration_table.csv"),
    row.names = FALSE
  )
  write.csv(
    integration$decision_matrix,
    file.path(output_dir, "tables", "11_decision_matrix.csv"),
    row.names = FALSE
  )
  writeLines(
    integration$verdict,
    file.path(output_dir, "logs", "12_final_verdict.txt")
  )
